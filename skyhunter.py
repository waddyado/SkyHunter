#!/usr/bin/env python3
# SKYHUNTER — RF AIRSPACE AWARENESS (TUI, libhackrf.py wrapper on Ubuntu/WSL/Windows via WSL)
...
def run(args, bands, live_status=None, on_alert=None):
    """Run the detector. Optional: live_status dict (updated each iteration), on_alert(label, ...) callback for web UI."""
    if HAVE_CURSES:
    ...
    if live_status is not None:
        ui.silent = True  # web UI mode: don't print status line to stdout
    ...
    if live_status is not None:
        live_status.update({"device": "ok", "error": None})
    ...
    try:
        ...
        while True:
            ...
            while (time.time() - t0) < dwell:
                ...
                if live_status is not None:
                    # Downsample spectrum to 256 points for web UI
                    n_spectrum = 256
                    idx = np.linspace(0, len(freqs_hz) - 1, n_spectrum).astype(int)
                    spectrum_freq_mhz = (freqs_hz[idx] / 1e6).tolist()
                    spectrum_db = psd_db[idx].tolist()
                    live_status.update({
                        "band": band,
                        "center_mhz": center / 1e6,
                        "floor_db": cur_floor,
                        "peak_db": cur_peak,
                        "min_floor_db": min_floor_db_seen,
                        "slice_mhz": [slice_lo / 1e6, slice_hi / 1e6],
                ...
                if len(psd_avg_buf) >= avg_frames:
                    ...
                    for a in det.process_psd(key, freqs_hz, psd_mean):
                        ...
                        msg = f"[ALERT] {label} @ {cf:.3f} MHz  ~{w:.1f} MHz  (mean +{mex:.1f} dB, peak +{pex:.1f} dB)"
                        ui.push_event(msg)
                        if on_alert:
                            on_alert({"label": label, "center_mhz": cf, "width_mhz": w, "excess_db_mean": mex, "excess_db_peak": pex, "message": msg})
                ...
                if args.floor_alert_enable and (min_floor_db_seen is not None):
                    ...
                    if sum(floor_hits_window) >= args.floor_persist_hits:
                        ...
                        # Skip floor alerts when width is 0 or tiny and we'd show UNKNOWN (avoids noise spam)
                        min_floor_alert_width_mhz = 2.0
                        skip = (label == "Signal Detected" and (est_w is None or est_w < min_floor_alert_width_mhz))
                        if not skip:
                            if pk_freq_mhz is None:
                                msg = f"[ALERT] {label}: floor {cur_floor:.1f} dB (+{cur_floor - min_floor_db_seen:.1f} dB)"
                                ui.push_event(msg)
                                if on_alert:
                                    on_alert({"label": label, "center_mhz": None, "width_mhz": est_w, "message": msg})
                            else:
                                msg = f"[ALERT] {label} @ {pk_freq_mhz:.3f} MHz  Peak {pk_db:.1f} dB  ~{est_w:.1f} MHz"
                                ui.push_event(msg)
                                if on_alert:
                                    on_alert({"label": label, "center_mhz": pk_freq_mhz, "width_mhz": est_w, "message": msg})
